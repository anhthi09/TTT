...
if(@$_FILES['Hinh']['error'] == 0){
    if(move_uploaded_file(@$_FILES['Hinh']["tmp_name"], "img_product/".@$_FILES['Hinh']["name"]))
        {
            $sql = "INSERT INTO `product` (`id`, `name`, `slug`, `soluong`, `gia`, `avatar`, `category`,`type`, `content`, `head`, `view`, `hot`, `created_at`, `updated_at`) 
            VALUES (NULL, '{$_REQUEST['TenSP']}', NULL,'{$_REQUEST['soluong']}', '{$_REQUEST['Gia']}', '{$_FILES['Hinh']['name']}', '{$_REQUEST['loaisp']}','{$_REQUEST['type']}', '{$_REQUEST['noidung']}', '0', '0', '0',current_timestamp() , current_timestamp())";
